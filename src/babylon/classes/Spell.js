...
export class Spell {
    constructor({ id, name, type, damage, manaCost, color, offset, guiTexture, scene, playerMesh, direction, cooldown, enemies = [] }) {
        this.id = id
        this.name = name
        this.type = type
        this.damage = damage
        this.manaCost = manaCost
        this.color = color || (type === "fire" ? new BABYLON.Color3(1, 0.3, 0) : new BABYLON.Color3(0.3, 0.3, 1))
    ...
    createUI() {
        addSpellButton(this.guiTexture, this.name, this.offset, () => {
            this.cast()
        })
    }
    ...
    cast() {
        ...
        const projectile = BABYLON.MeshBuilder.CreateBox(`${this.id}_projectile`, { size: 0.3 }, this.scene)
