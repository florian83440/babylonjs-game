...
import { createManaBar, updateMana } from './gui/manaBar'
...
export function createScene(engine, canvas) {
    ...
    for (let x = 0; x < mapSize / 2; x++) {
        for (let z = 0; z < mapSize / 2; z++) {
            const tile = BABYLON.MeshBuilder.CreateGround(`tile_${x}_${z}`, { width: size, height: size }, scene)
            ...
            const lines = BABYLON.MeshBuilder.CreateLines(`border_${x}_${z}`, {
                points: [
    ...
    let playerX = tileX, playerZ = tileZ
    ...
    const camera = new BABYLON.ArcRotateCamera("camera", -Math.PI / 2, Math.PI / 3, 20, box.position, scene)
